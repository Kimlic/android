...
interface CompanyDao {
    ...
    @Query("SELECT C.id, C.user_id, C.address, C.details, C.email, C.logo, C. name, C.phone, C.website, C.status, C.inserted_at  FROM company as C INNER JOIN user as U ON user_id = U.id AND status=:status WHERE u.account_address=:accountAddress")
